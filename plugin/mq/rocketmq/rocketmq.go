...
func (r *rocketmqBroker) getPushConsumer(groupName string) (rocketmq.PushConsumer, error) {
	...
	if mode, ok := r.opts.Context.Value(consumerModeKey{}).(MessageModel); ok {
		//ropts = append(ropts, consumer.WithConsumerModel(consumer.Clustering))
		ropts = append(ropts, consumer.WithConsumerModel(consumer.MessageModel(mode)))
	}
	//ropts = append(ropts, consumer.WithConsumerOrder(true))
	if fromWhere, ok := r.opts.Context.Value(fromWhereKey{}).(ConsumeFromWhere); ok {
		ropts = append(ropts, consumer.WithConsumeFromWhere(consumer.ConsumeFromWhere(fromWhere)))
	}
